---
layout: post
title: Value stream mapping in practice
tags:
  - process
author: Marc Evers
image: 
---

# what is value stream mapping

+ why / what value does it bring?
(zoom out / take systems view)

link to [productivity is systemic]()
not an individual thing; not about producing code faster
"being productive" is about flow, from concept to cash (from Poppendieck)

## who to involve & scope

Ideally, concept to cash (Poppendieck)

Need to have everyone involved in the room, including business, product management, UX, ...

Still can be useful to map the flow for a narrower part, e.g. with just the development team. On one hand suboptimal (bottleneck could be somewhere else); other hand: focus on what is within control of the team
And: doing small improvements to make delivery more smooth improves the 'inner loop', which does have an impact on the overall flow.


## agile sucks

agile/scrum/SAFe tends to create stricter team boundaries, e.g. work is mediated by a product owner. This risks more local optimization. 
Doing VSM with product management/business involved helps everyone see the whole. Enables developers to better understand what is valuable.

# Example 

fictitious but highly inspired by some of our clients


# Learn more? 

See series of posts by Johanna Rothman

cooperative vs collaborative teams (Johanna)

# Value Stream Mapping in practice

Some examples of what we have seen
- productizing research in a research institute
- flow stalling at pull request reviews 
- granularity of work (3)

Cooperative teams tend to focus more on local optimization; VSM helps to see the whole, stimulate more collaboration.

Being busy / utilized is not the goal (although you might feel that you get paid for doing stuff). Does not mean just sitting there (although just sitting there and observing what's happening can be very useful!) but stepping in to help a colleague if that improves overall flow. If you just continue working on your part, you will just increase work in progress

VSM is a way of zooming out; helps participants to look at the whole, not just their part

VSM is a helps starting conversations about: optimize delivery of value; prevent local optimization; work more collaboratively


# considerations

- objectives
- scope?
- what it does, and what it does not address
  - time lost in 'meetings' (reduced focus/flow time)

cumulative flow diagrams can help 


https://www.jrothman.com/mpd/2026/03/how-to-use-value-stream-maps-to-reinforce-agility-effectiveness-part-1/
https://www.jrothman.com/mpd/2026/04/how-to-use-value-stream-maps-to-reinforce-agility-effectiveness-part-2-cooperative-teams/
https://www.jrothman.com/mpd/2026/04/how-to-use-value-stream-maps-to-reinforce-agility-effectiveness-part-3-collaborative-teams/
https://www.jrothman.com/mpd/2026/04/how-value-stream-maps-prompt-us-to-ask-more-questions-part-4/

https://www.jrothman.com/mpd/2026/06/how-to-use-value-stream-maps-to-see-where-ai-creates-bottlenecks-part-1/

Poppendieck, Lean Software Development

https://andrewmurphy.io/blog/if-you-thought-the-speed-of-writing-code-was-your-problem-you-have-bigger-problems

https://substack.jurgenappelo.com/p/value-stream-examples

