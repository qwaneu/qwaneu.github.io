...
I decided to port the microprint library made with Stephan Eggermont to the web framework that we use - Phoenix Liveview. Now the micrprints live inside the web application, as we are building it. This helps us make better decisions about where we need to refactor, where we need more tests, where we need to learn more etc. This way we can keep an eye on the code as it develops. Iterative work and short feedback cycles remain valuable, if not more so, with AI Augmented development.
